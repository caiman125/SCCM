...
# Function to get folder sizes up to 4 levels deep
function Get-FolderSizesUpToLevel4 {
    param (
        [string]$rootPath
    )
    #$rootPath.Replace('\','\\')
    ...
    $folders = Get-ChildItem -Path $rootPath -Directory -Recurse | Where-Object { $_.FullName -match $matchPath } 
...
Get-FolderSizesUpToLevel4 -rootPath $rootDir | Format-Table -AutoSize | Out-String
